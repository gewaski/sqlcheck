...
cat $2.tpl all.tpl | sort -u > all.tpl.new
cat all.tpl.new $confirmed | sort | uniq -d > dup.tmp.tpl
cat all.tpl.new dup.tmp.tpl | sort | uniq -u > all.tpl
cat all.tpl | grep -E ^SELECT\|^DELETE | grep FROM | sed 's/.*FROM\s*\(\S*\).*/\1--->\0/g' > $2.tpl.unresolved
cat all.tpl | grep -E ^UPDATE | sed 's/^UPDATE\s*\(\S*\)\s*SET.*/\1--->\0/g' >> $2.tpl.unresolved

awk -F"--->" 'BEGIN{old=""} {if(old==$1) {print $2} else {print "\n*******************************************\n"$1"\n*******************************************\n"$2; old=$1}}' $2.tpl.unresolved > /var/ftp/pub/from_$2_to_`date +%y%m%d`.unresolved

# cleanup
rm $2 $2.tpl.unresolved all.tpl.new dup.tmp.tpl format.result $2.tpl all.tpl -f
